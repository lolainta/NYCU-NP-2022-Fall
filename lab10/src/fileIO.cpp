...
int fileIO::readFiles(string filePath) {
    ...
    int uncompressedSize = 4 + (4 + 2) * 1000 + contentCounter;

    // int compressedSize = ht.compressFile((uint8_t *)this->payload, uncompressedSize);
    // cout << "[FileIO] Compression rate: " << (double)compressedSize / uncompressedSize * 100 << "%" << endl;
...
int fileIO::writeFiles(string filePath) {
    ...
    // huffmantool ht;
    cout << "Start to uncompressed" << endl;
    // int uncompressedSize = ht.decompressFile((uint8_t *)this->payload, this->payloadSize);
